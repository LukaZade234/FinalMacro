...
5. **Unused or mis-spent daily budget** — `$ot` is **playable by hand** (Run page → **Play $ot**, `macro/ot_game.py`) and now exploits **Extra Chance**: **915 base SP a board across the 27 real boards, 100.2% of the all-ships ceiling, vs the 745.5 those boards actually paid** (+168.9, t = 3.72), 7 boards cleared outright, on a balance the logs show sitting at `$ot 3 unused` run after run. Ten hand-played games confirmed the mechanic and the solver has been tried on them, so the remaining reason it is **out of play-all and out of the after-refill auto-play** is that no game has yet been played *under the new rules* — do that first, then promoting it is one line in `PLAYABLE_MINIGAMES` plus a `_play_batches` call. ~~Perk 9 static filter~~ — opt-in adaptive threshold spends the daily clicks by EV.
...
- ~~**`$ot` solver**~~ — `macro/ot_solver.py` + `macro/ot_replay.py` + `scripts/ot_bakeoff.py`, and now `macro/ot_game.py` behind a **Play $ot** button on all four Run shells, playing the **Extra Chance** line (915 SP a board on the 27 real boards, 100.2% of ceiling, 7 cleared outright). **Still manual on purpose:** `$ot` is not in `PLAYABLE_MINIGAMES`, so play-all skips it and it never runs itself after the daily refill. Promote it once a batch has been played under the new rules and the logs show `(Extra chance)` lines.
