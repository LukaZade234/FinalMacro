...
15. ~~**`$ot` Phase 2**~~ (solver, harness and a manual Play $ot button shipped; auto-play still off) / **use parsed `$settings`/`$bonus` in decisions** / **full daily autonomy** / **Phase D** — real projects.
...
5. **Unused or mis-spent daily budget** — `$ot` is now **playable by hand** (Run page → **Play $ot**, `macro/ot_game.py`), worth roughly **1,000–1,150 SP a game** against the ~600 a human scores, on a balance the logs show sitting at `$ot 3 unused` run after run. It is deliberately **out of play-all and out of the after-refill auto-play** until the solver has been tried on real boards; promoting it is one line in `PLAYABLE_MINIGAMES` plus a `_play_batches` call. ~~Perk 9 static filter~~ — opt-in adaptive threshold spends the daily clicks by EV.
...
- ~~**`$ot` solver**~~ — `macro/ot_solver.py` + `macro/ot_replay.py` + `scripts/ot_bakeoff.py`, and now `macro/ot_game.py` behind a **Play $ot** button on all four Run shells. **Still manual on purpose:** `$ot` is not in `PLAYABLE_MINIGAMES`, so play-all skips it and it never runs itself after the daily refill. Promote it once real boards confirm the solver.
...
- ~~**`$ot` — hard, already on the daily-loop list.**~~ **Solver and game loop done; auto-play deliberately still off.** Battleship on a 5×5: ships are free, 4 blue clicks. Colblitz enumerate placements with a bitmask DFS then run a two-phase policy, Phase 1 using a **learned** scorer they do not publish. Rare-ship SP is 76 / 104 / 150 / 500 (Light / Dark / Red / Rainbow), not a flat ~90 — confirmed, it is just `SPHERE_BASE_SP` plus the `$oh` light/dark means.
  ...
  **Measured dead end: one-ply lookahead.** Scoring a probe by what it *unlocks* (`Σ P(outcome) × [SP + harvest]`) scored **696 vs 1043** on the known boards. Not a bug — the harvest term is a *stock* (SP eventually collectable) added to a *flow* (SP from this click), so it double-counts value the policy would have reached anyway and chases certainty over payout. Doing it properly needs a rollout to the end of the game, not one ply. It is kept in `PROBE_POLICIES` so the result is reproducible; do not re-derive it. This is the same shape as the reverted `$oc` hunt lookahead.
  **Still open:** promoting `$ot` out of manual-only (play-all + after-refill auto-play), and confirming Extra Chance (`EXTRA_CHANCE` is off; the harness honours it, so it can be priced the day a real board settles it). ~50 real boards would settle λ properly.
